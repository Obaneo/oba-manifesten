...
This chapter specifies the **lifeworld layer** as a **test layer**: a validation interface that binds macro-invariants to experienced
reality under evidence, publication, and contestability rails. It does not add new canonical definitions; it translates and checks
claims (P/E/C, caps, ΔM, and B/T/N/L/R) against **household-observable experience** and defines hard stops when the test cannot be
executed. The lifeworld layer is therefore a **legitimacy and anti-washing surface**: if it is absent, non-reproducible, or selective,
the implementation is incomplete or drifting—even when macro claims appear internally consistent.
